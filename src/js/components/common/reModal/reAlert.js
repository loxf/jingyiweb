...
class ReAlert extends Component {
    ...
    static alert(text,okCallBack,config) {
        this.setState({
            ...
            clickClose:config&&typeof(config.clickClose)!='undefined'?config.clickClose:true,
        })
    }
    ...
    //确定
    ok() {
        this.state.clickClose&&this.close();
            ...
            case 'confirm':
                return <div>
                    ...
                    <div className={style.buttonContainer}>
                        <button className={style.cancel} onClick={this.cancel}>{this.state.cancelText}</button>
                        <button className={style.ok} onClick={this.ok}>{this.state.okText}</button>
                    </div>
                </div>;
            case 'alert':
                return <div>
                    <div className={this.getAlertContent()}>{this.state.text}</div>
                    <div className={style.buttonContainer}>
                        <button className={style.singleButton} onClick={this.ok}>{this.state.okText}</button>
                    </div>
                </div>;
            case 'component':
            case 'onlyComponent':
    ...
    renderCloseContainer(){
        return this.state.type!='onlyComponent'?<div className={style.closeContainer}>
                <button className={style.close} onClick={this.cancel}></button>
            </div>:''
    }
